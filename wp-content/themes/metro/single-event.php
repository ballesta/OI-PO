...
<?php } ?>
	          ...
	          if ( current_user_can( 'edit_post', $post->ID ) )
	      	    edit_post_link( __('Modifier', 'om_theme'), 
	...
	<?php if($post_sidebar_show) { ?>
		...
		<div class="block-3 no-mar sidebar">
			<?php
				...
				//	get_sidebar();
				//}
				//dynamic_sidebar('sidebar-actualite'); 
			?>
			<ul>
			<?php if (get_field('lien_externe')): ?>		
				<h2 class="widget-header">En savoir+</h2>		
				<ul>
					<li>
						<a href="<?php the_field('lien_externe');?>">
							<?php the_field('texte_lien_externe');?>
						</a>	
					</li>
				</ul>			
			<?php endif; ?>
			<?php if ( is_active_sidebar( 'sidebar-actualite' ) ) : ?>
				<?php dynamic_sidebar( 'sidebar-actualite' ); ?>
			<?php else: ?>	
				sidebar-actualite?<br>
			<?php endif; ?>
			</ul>
