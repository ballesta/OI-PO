	    	...
	    	<article>		
				<div class="tbl-bottom">
					<div class="tbl-td">
						...
						<?php if($format == 'quote') { ?>
							<div class="clear"></div>
							<p class="post-title-comment" style="margin:0 0 3px 0">
									&mdash; <?php the_title(); ?>
							</p>
						<?php } ?>
					</div>
...
/*
	// Related posts
	$relationship_field_name = 'ouvrages_bibliographie';
	$relationship_title='Références bibliographiques';
	$related = get_field($relationship_field_name);

	if( $related ) 
	{
		echo '<hr />';
	    echo "<h3>$relationship_title</h3>";
		echo '<ul>';
		foreach( $related as $post ) 
		{
			...
			//print_r($post);
			//echo '</pre>';
			// Le post devient le Post courant, et peut être affiché 
			setup_postdata($post); 
			echo	
				 '	<li>',
...
?>
			...
			<?php
	          if ( current_user_can( 'edit_post', $post->ID ) )
	      	    edit_post_link( __('Modifier', 'om_theme'), '<div class="edit-post-link">[', ']</div>' );
	    	?>
