...
*/ 
	...
	class WpFastestCache{
	    	...
	    	foreach ($crons as $cron_key => $cron_value) {
	    		foreach ( (array) $cron_value as $hook => $events ) {
	    			if(preg_match("/^wp\_fastest\_cache(.*)/", $hook, $id)){
		    			...
		    			if(!$id[1] || preg_match("/_Preload_/i", $id[1])){
		...
		public function set_preload(){
			if(!get_option("WpFastestCachePreLoad")){

				$data = array("homepage" => "",
					"single" => 0, // for offset. How much post has been cached?
					"tags" => "",
					"categories" => ""
				);

				$json = json_encode($data);
				add_option("WpFastestCachePreLoad", $json, null, "yes");
			}

			wp_schedule_single_event(time() + 10, $this->slug()."_Preload_".(time()+10));
		}

		public function create_preload_cache(){
			if($data = get_option("WpFastestCachePreLoad")){
				$pre_load = get_option("WpFastestCachePreLoad");
			}
			

    		$recent_posts = wp_get_recent_posts(array(
									'numberposts' => 2,
								    'offset' => $pre_load->single,
								    'orderby' => 'ID',
								    'order' => 'DESC',
								    'post_type' => 'post',
								    'post_status' => 'publish',
    		...
    		if(count($recent_posts) > 0){
    			...
    			$data["single"] = "Completed";
    		}

    		update_option("WpFastestCachePreLoad", $values);
			...
			if($url){
				$response = wp_remote_get($url, array('timeout' => 10, 'headers' => array("cache-control" => array("no-store, no-cache, must-revalidate", "post-check=0, pre-check=0"),'user-agent' => "WP Fastest Cache Pre-Load Bot")));
				...
				wp_schedule_single_event(time() + 30, $this->slug()."_Preload_".(time()+30));
			}
			die();
		}
		...
		protected function getMobileUserAgents(){
			...
			*/
			...
			return "iphone|midp|sony|symbos|nokia|samsung|mobile|epoc|ericsson|panasonic|philips|sanyo|sharp|sie-|portalmmm|blazer|avantgo|danger|palm|series60|palmsource|pocketpc|android|blackberry|playbook|ipad|ipod|iemobile|palmos|webos|googlebot-mobile|bb10|xoom|p160u|nexus|touch|SCH-I800|opera\smini|SM-G900R4|LG-|HTC|GT-I9505|WAP-Browser";
		}
