...
Class PMS_Submenu_Page_Addons extends PMS_Submenu_Page {
    ...
    */
    ...
    function add_on_save_serial(){
        ...
        if( !empty( $pms_add_on_slug ) ){
            if( !empty( $pms_serial_value ) ) {
                ...
                $response = PMS_Submenu_Page_Addons::add_on_check_serial_number( $pms_serial_value, $pms_add_on_slug );
            }
            else
    ...
    static function add_on_check_serial_number( $serial, $add_on_slug ){
        $remote_response = wp_remote_get( 'http://updatemetadata.cozmoslabs.com/checkserial/?serialNumberSent='.$serial );
