 ...
 */
    ...
    function pms_pb_before_paypal_redirect( $paypal_link, $payment_data, $settings ) {

        if( !isset( $payment_data['payment_id'] ) )
            return;

        set_transient( 'pms_pb_pp_redirect_' . $payment_data['payment_id'], $paypal_link, DAY_IN_SECONDS );

        // save payment ID as global to use when getting the redirect back link (on 'wppb_register_redirect')
        global $payment_data_id;
        $payment_data_id = $payment_data['payment_id'];
