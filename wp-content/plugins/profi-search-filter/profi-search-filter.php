<?php
	...
	*/
	define( 'SF_CURRENT_VERSION', '1.76' );
	if( !session_id() )
		session_start();
	...
	require_once( SF_DIR . 'includes/widget.php' );
	...
	function sf_init_searchform( $attr, $preset="" ){
		ob_start();
		if( !empty( $preset ) ){
			$preset = preg_replace( '^(&#8221;)|(&#8220;)|(&#8243;)^', '"', $preset );
			$preselect = json_decode( $preset, true );
			$_POST['data'] = $preselect;
		}
	...
	/*
	add_shortcode( 'search-related', 'sf_search_related' );
	function sf_search_related( $content ){
		if( !isset( $_SESSION['sf'] ) )
			return;
		ob_start();
		require( SF_DIR . 'includes/shortcode_search_related.php' );
		$output_string=ob_get_contents();
		ob_end_clean();
		return $output_string;
	}
	*/
	add_shortcode( 'search-form-results', 'sf_init_searchform_results' );
	function sf_init_searchform_results( $attr ){
		ob_start();
		require( SF_DIR . 'includes/shortcode-results.php' );
		$output_string=ob_get_contents();
		ob_end_clean();
		return $output_string;
	}
	...
	function sf_content_filter( $sf_where, &$wp_query ){
		...
		if( $wp_query->get( 'sf-title' ) || $wp_query->get( 'sf-meta' ) || $wp_query->get( 'sf-content' )  || $wp_query->get( 'sf-excerpt' ) ):
			...
			if( !empty( $concat_string ) ):
				$sf_add_where = ' AND (';
				$concat_string = "CONCAT( \"\" " . $concat_string . ") LIKE '%" . esc_sql( $wpdb->esc_like( $search_term ) ) . "%' ";
	...
	function create_hierarchical_select( $terms, $start = 0 ){
		if( $start == 0 )
			$terms = order_terms_hierarchical_array( $terms );
		$select = array();
		$sstring = '';
		foreach( $terms as $term ):
			$sstring .= '<option value="' . $term->term_id . '">' . $term->name . '</option>' . PHP_EOL;
			if( count( $term->children ) > 0 ):
				if( !isset( $select[ $term->term_id ] ) )
					$select[ $term->term_id ] = array();
				$select[ $term->term_id ] = create_hierarchical_select( $term->children, 1 );
				foreach( $select[ $term->term_id ] as $key => $val ):
					if( $key != $term->term_id )
						$select[ $key ] = $val;
				endforeach;
				$select[ $term->term_id ] = $select[ $term->term_id ][ $term->term_id ];
			endif;
		endforeach;
		$select[ $terms[0]->parent ] = $sstring;
		return $select;
	}
	...
	function sf_get_all_dates( $posttype, $type, $column ){
		global $wpdb;
		$sql = "SELECT " . $type . "( " . $column . " ) AS `date`
									FROM " . $wpdb->posts . "
									WHERE 
										post_type IN ('" . implode( "','", $posttype ) . "') AND 
										post_status = 'publish' 
										GROUP BY " . $type . " (  " . $column . "  ) 
										ORDER BY " . $type . "(  " . $column . "  ) ASC";
