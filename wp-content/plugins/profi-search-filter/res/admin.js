jQuery( document ).ready( function(){
	...
	jQuery( document ).on( 'click', '.field.filter>div', function(){
		...
		if( typeof( attr.placeholder ) != 'undefined' )
		...
		if( attr.type == 'fulltext' || attr.type == 'input' ){
			txt += '<label>' + objectL10n.placeholder + ':</label><input name="placeholder" value="' + placeholder + '" /><br />';
		}
		...
		if( attr.type == 'orderby' ){
			if( typeof attr.orderby != 'undefined' ){
				...
				jQuery( '#sf-dlg .sf-orderby input[type="checkbox"]' ).each( function(){
					...
					jQuery( this ).parent().parent().find( 'input.sf-orderbylabel' ).val( attr.orderbylabel[Index] );
					if( typeof( attr.orderbynumerical[Index] ) != 'undefined' )
						jQuery( this ).parent().parent().find( 'select' ).val( attr.orderbynumerical[Index] );
