...
class Searchfrom_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		?>
			...
			<form method="<?php if( isset( $instance['method'] ) ) echo $instance['method']; else echo 'POST'; ?>" data-withfilter="<?php if( isset( $instance['withfilter'] ) ): echo $instance['withfilter']; else: echo '0'; endif; ?>" data-autoload="<?php echo $instance["autoload"] ?>" action="<?php echo get_permalink( $instance['target'] ); ?>">
		...
		<?php			
			if( $element['type'] == 'hierarchical-taxonomies' ):
					...
					if( isset( $_POST['search-id'] ) && isset( $_POST[ $key ] ) ):
					?>
					<script>
						jQuery( document ).ready( function(){
							sf_loadHierarchie( <?php echo $key; ?>, '<?php echo $_POST[ $key ]; ?>' );
						});
					</script>
					<?php
					endif;
			...
			<select id="sf-field-<?php echo $key; ?>" name="<?php echo $key; ?>"><option value=""><?php if( isset( $element['all_options'] ) ) echo $element['all_options']; ?></option><?php
				...
				elseif( $data_type == 'others' ):
					...
					if( $data_value == 'pub_d' ):
						...
						foreach( $days as $d ):
						?>
						<option <?php if( isset( $_POST[ $key ] ) && $_POST[ $key ] == $d->date ) echo 'selected="selected" '; ?>value="<?php echo $d->date; ?>"><?php echo $d->date; ?></option>
						<?php
						endforeach;
					endif;
					if( $data_value == 'pub_m' ):
						...
						foreach( $months as $m ):
						?>
						<option <?php if( isset( $_POST[ $key ] ) && $_POST[ $key ] == $m->date ) echo 'selected="selected" '; ?>value="<?php echo $m->date; ?>"><?php echo $m->date; ?></option>
						<?php
						endforeach;
					endif;
					if( $data_value == 'pub_y' ):
						...
						foreach( $years as $y ):
						?>
						<option <?php if( isset( $_POST[ $key ] ) && $_POST[ $key ] == $y->date ) echo 'selected="selected" '; ?>value="<?php echo $y->date; ?>"><?php echo $y->date; ?></option>
						<?php
						endforeach;
					endif;
			...
			elseif( $element['type'] == 'input' ):
				?>
				<input placeholder="<?php if( isset( $element['placeholder'] ) ) echo $element['placeholder']; else echo $element['fieldname']; ?>" id="sf-field-<?php echo $key; ?>" name="<?php echo $key; ?>" <?php if( isset( $_POST[ $key ] ) ) echo ' value="' . $_POST[ $key ] . '" '; ?> />
				<?php	
			...
			?>
				<div class="sf-widget-fulltext-wrapper">
					<input <?php if( isset( $_POST[ $key ] ) ) echo ' value="' . $_POST[ $key ] . '" '; ?>placeholder="<?php if( isset( $element['placeholder'] ) ) echo $element['placeholder']; else echo $element['fieldname']; ?>" name="<?php echo $key; ?>" />
				</div>
			<?php
			elseif( $element['type'] == 'orderby' ): ?>
				...
				$index = substr( $index[ count( $index ) - 1 ], 0, -1 );?>
					<option <?php if( isset( $_POST['_orderby'] ) && $e == $_POST[ '_orderby' ] ) echo ' selected="selected" '; ?>value="<?php echo $e; ?>"><?php echo $element['orderbylabel'][ $index ]; ?></option>
				<?php endforeach; ?>				
				</select>
			<?php
			...
			elseif( $element['type'] == 'date' ):
				...
				if( isset( $element['ui'] ) && $element['ui'] != 'none' )
					wp_enqueue_style('sf-datepicker-css',
						...
						false);
			?>
				<input placeholder="<?php _e( 'From', 'sf' );?>" data-dateformat="<?php echo $element['dateformat']; ?>" class="sf-date <?php if( isset( $element['searchtype'] ) && $element['searchtype'] == 'between' ): echo 'sf-date2 first'; endif; ?>" value="<?php if( isset( $_POST[ $key ][0] ) ) echo $_POST[ $key ][0]; ?>" name="<?php echo $key; ?>[]" />
			<?php
				if( isset( $element['searchtype'] ) && $element['searchtype'] == 'between' ):
				?>
				<input placeholder="<?php _e( 'To', 'sf' );?>" data-dateformat="<?php echo $element['dateformat']; ?>" class="sf-date sf-date2 second" name="<?php echo $key; ?>[]" value="<?php if( isset( $_POST[ $key ][1] ) ) echo $_POST[ $key ][1]; ?>" />
				<?php
				endif;
			elseif( $element['type'] == 'range' ):
			?>
	...
	function form( $instance ) {
		...
		$defaults = array( 'button' => __( 'Search', 'sf' ), 'title' => __( 'Search', 'sf' ), 'method' => 'POST' );
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'method' ); ?>"><?php _e( 'Method', 'sf' ); ?>:</label>
			<select id="<?php echo $this->get_field_id( 'method' ); ?>" name="<?php echo $this->get_field_name( 'method' ); ?>">
				<option value="POST" <?php selected( 'POST', $instance['method'] ); ?>>POST</option>
				<option value="GET" <?php selected( 'GET', $instance['method'] ); ?>>GET</option>
			</select>
		</p>
		<p>
