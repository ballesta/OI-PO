<?php
	...
	add_action('wp_ajax_sf-getterm', 'sf_ajax_getterm');
	add_action('wp_ajax_nopriv_sf-getterm', 'sf_ajax_getterm');
	function sf_ajax_getterm(){
		$term = explode( '|', $_POST['term'] );
		$term = $term[ count( $term ) - 1 ];
		if( $term == "" ){
			echo json_encode( array( 'term' => 0, 'children' => array() ) );
			die();
		}
		$args = array(
			'parent'	=> (int) $term
		...
		$tmp = array( 'term' => $term, 'children' => array() );
		foreach( $terms as $val )
			$tmp['children'][] = array( 'term_id' => $val->term_id, 'name' => $val->name );
	...
	function sf_do_search( $exclude = array() ){
		...
		foreach( $data['post'] as $key => $val ){
			if( ! is_array( $val ) ){
				$data['post'][ $key ] = stripslashes( $val );
			} else {
				foreach( $val as $subkey => $subval )
					$data['post'][ $key ][ $subkey ] = stripslashes( $subval );
			}
		}
		...
		if( isset( $_POST['data']['wpml'] ) ):
			global $sitepress;
			//WPML
			if( isset( $sitepress->switch_lang ) )
		...
		$field = apply_filters( 'sf-field', $field, $_POST );
		...
		foreach( $field['fields'] as $key => $val ):
			if( $val['type'] == 'fulltext' && !empty( $_POST['data'][ $key ] ) ):
				$fulltext = $_POST['data'][ $key ];
			endif;
		endforeach;
		$fulltext = apply_filters( 'sf-fulltext-text', $fulltext, $field, $_POST );

		$args = sf_generate_args( $field, $exclude );
		$data['result'] = array();
		...
		if( $query->have_posts() ):
			while( $query->have_posts() ): $query->the_post();
				...
				ob_end_clean();
				if( $field['highlight'] == 1 && ! empty( $fulltext ) ):
					...
					$template_single = stripslashes( preg_replace( '^#the_title#^', preg_quote( get_the_title() ), $template ) );
					$template_single = stripslashes( preg_replace( '^#the_excerpt#^', preg_quote( get_the_excerpt() ), $template_single ) );
					$template_single = stripslashes( preg_replace( '^#the_content#^', preg_quote( get_the_content() ), $template_single ) );
				endif;
				// bb
				$auteur = compose_auteur($field);
				if ($auteur == ''):
					// Aucun auteur sur les 3 possibles, mettre l'auteur de l'article par defaut
					$auteur = get_the_author();
				endif;	
				$template_single = stripslashes( 
					preg_replace( '^#the_author#^', 
				                   preg_quote( $auteur ), 
				                   //preg_quote( get_the_author() ), 
					$template_single ) );
				$template_single = stripslashes( preg_replace( '^#the_date#^', preg_quote( get_the_date() ), $template_single ) );
				$template_single = stripslashes( preg_replace( '^#the_permalink#^', preg_quote( get_permalink() ), $template_single ) );
				...
				// bb
				$livrables = affiche_livrables();
				$template_single = preg_replace( '^#meta_' . preg_quote( 'livrables' ) . '#^',
  				                                 $livrables, $template_single );					
				...
				$data['result'][] = '<li data-postID="' . get_the_ID() . '">' . apply_filters( 'sf-results-single-result', $template_single ) . '</li>';
			endwhile;
		endif;
		$data['result'] = apply_filters( 'sf-all-results-html', $data['result'] );
		...
		if( defined( 'ICL_LANGUAGE_CODE' )  ):
			//WPML
			global $sitepress;
			if( isset( $sitepress->get_current_language ) ):
		...
		wp_reset_query();
		return apply_filters( 'sf-all-data', $data );
	}
	
	// bb +++++++
	function compose_auteur($field)
	{
		//OI::affiche($field, 'Realisation meta');
		$post_type = get_post_type();
		//echo $post_type, '<hr>';
		if ($post_type == 'realisation'):
			//if( isset( $field['meta'] ) && is_array( $field['meta'] ) ):
				// Présence des champs sur les auteurs
				//OI::affiche($field['meta'], 'Realisation meta');
				$auteur_externe = get_post_meta( get_the_ID(), 
				                                'auteur_externe', true );
				$auteur_inscrit_sur_le_site = get_post_meta( get_the_ID(), 
				                                             'auteur_inscrit_sur_le_site', true );
				$auteur_membre = get_post_meta( get_the_ID(), 
				                                'auteur_membre', true );
				//echo "auteur_inscrit_sur_le_site: $auteur_inscrit_sur_le_site<hr>";
				if ($auteur_externe != ''):
					// Auteur externe renséigné
					$auteur = $auteur_externe;
				elseif ($auteur_inscrit_sur_le_site > 0):
				    // Utilisateur du site renseigné
					$auteur = get_the_author_meta( 'display_name' , $auteur_inscrit_sur_le_site );
				    if ($auteur == ''):
						//echo '<hr>';
						//die("Auteur $auteur_inscrit_sur_le_site non trouvé");
					endif;	
					//OI::affiche($auteur_membre, 'auteur_membre');
					if (count($auteur_membre) > 0):
						$membre_id = $auteur_membre[0];
						$membre_post = get_post($membre_id);
						//OI::affiche($membre_post);
						$nom = $membre_post->post_title;
						$auteur .= " ($nom)";
					endif;
				else:
                $auteur = '';			
		...
		//echo "Auteur: $auteur<hr>";
		return $auteur;
	}

	// bb
	function affiche_livrables()
	{ 
	...
	function sf_generate_args( $field, $exclude ){
		$args = array(
		...
		*/
			...
			if( isset( $_POST['data'][ $key ] ) ):
				...
				*/
				if( $data_type == 'others' ):
					...
					elseif( $data_value == 'pub_d' ):
						$args['date_query']['inclusive'] = true;
						$args['date_query']['day'] = $_POST['data'][ $key ]['val'];
					elseif( $data_value == 'pub_m' ):
						$args['date_query']['inclusive'] = true;
						$args['date_query']['month'] = $_POST['data'][ $key ]['val'];
					elseif( $data_value == 'pub_y' ):
						$args['date_query']['inclusive'] = true;
						$args['date_query']['year'] = $_POST['data'][ $key ]['val'];
					elseif( $data_value == 'mod_d' ):
						$args['date_query']['inclusive'] = true;
						$args['date_query']['day'] = $_POST['data'][ $key ]['val'];
						$args['date_query']['column'] = 'post_modified';
					elseif( $data_value == 'mod_m' ):
						$args['date_query']['inclusive'] = true;
						$args['date_query']['month'] = $_POST['data'][ $key ]['val'];
						$args['date_query']['column'] = 'post_modified';
					elseif( $data_value == 'mod_y' ):
						$args['date_query']['inclusive'] = true;
						$args['date_query']['year'] = $_POST['data'][ $key ]['val'];
						$args['date_query']['column'] = 'post_modified';
					endif;
				...
				*/
				elseif( $data_type == 'tax' ):
					...
					/** Select Field */
					elseif(  $val['type'] == 'hierarchical-taxonomies' && $_POST['data'][ $key ]['val']  != "" ):
						$_POST['data'][ $key ]['val']  = explode( '|', $_POST['data'][ $key ]['val'] );
						$hierarchical_val  = $_POST['data'][ $key ]['val'][ count( $_POST['data'][ $key ]['val'] ) - 1 ];
						if( empty( $hierarchical_val ) && count( $_POST['data'][ $key ]['val'] ) > 1 )
							$hierarchical_val  = $_POST['data'][ $key ]['val'][ count( $_POST['data'][ $key ]['val'] ) - 2 ];
							
						$args['tax_query'][] = array( 
							'taxonomy'	=> $data_value, 
							'terms'		=> (int) $hierarchical_val
						);
		...
		if( isset( $_POST['data']['orderby'] ) ):
					...
					if( isset( $val['orderbynumerical'][ $index ] ) && $val['orderbynumerical'][ $index ] == '123' )
						$orderbynum = 'meta_value_num';
					else
						$orderbynum = 'meta_value';
		...
		if( isset( $_POST['data']['_orderby'] ) ):
					preg_match_all( '^(.*)\[(.*)\|(.*)\|(.*)\]^', $_POST['data']['_orderby']['val'], $matches );
					$data_type = $matches[1][0];
					$data_value = $matches[2][0];
					$order = $matches[3][0];
					$index = $matches[4][0];
