...
function bbptoolkit_check_bbpms_active() {
	// If bbP manage subscriptions is active and old version of it is detected, this might give conflicts, so stop here !
	if ( is_plugin_active( 'bbp-manage-subscriptions/bbp-manage-subscriptions.php' ) ) {
		$plugindata = get_plugin_data( ABSPATH . PLUGINDIR . '/bbp-manage-subscriptions/bbp-manage-subscriptions.php', false, false );
		$bbpms_version = $plugindata['Version'];
		if ($bbpms_version) {
			return $bbpms_version;
		} else {
			return '1.0.0';
		}
	}
