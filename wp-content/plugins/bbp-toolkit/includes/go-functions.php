...
add_filter('bbp_before_has_replies_parse_args', 'bbptoolkit_has_replies' );
...
function bbptoolkit_secure_profile() {
	if ( !is_user_logged_in() ) {
		...
		if ($bbptoolkit_sec_profile) {
			...
			get_header();
			echo '<br><br><h3 class="bbpt-sec-profile">Sorry, You must be logged in to view this area.<br><br>' ;
			header( "refresh:5;url=".$bbptoolkit_sec_profile_path ); 
			echo 'You will be redirected in about 5 secs. If not, click <a href="'.$bbptoolkit_sec_profile_path.'">here</a>.<h3><br><br><br>';
			exit;
		}
	}
}
