...
function UTF8_DB_Converter_DoIt() {
    ...
    while ( $row = mysql_fetch_row($result) )
    {
       ...
       while ( $row3 = mysql_fetch_row($result3) )
       {
          ...
          //bb $skipped_field_types = array('char', 'text', 'blob', 'enum', 'set');
		  // retiré "blob"
          $changed_fields_types = array('char', 'text', 'enum', 'set');
          
          foreach ( $changed_fields_types as $type )
          {
             if ( strpos($field_type, $type) !== false )
             {
				...
				$sql5 = "ALTER TABLE $table MODIFIY `$field_name` CHARACTER SET utf8 COLLATE utf8_general_ci";
				print $sql5 . '<br>';
                if ( !($result4 = mysql_query($sql4, $connection)) )
                {
                   print '<span style="color: red;">SQL Error: <br>' . mysql_error() . "</span>\n";
