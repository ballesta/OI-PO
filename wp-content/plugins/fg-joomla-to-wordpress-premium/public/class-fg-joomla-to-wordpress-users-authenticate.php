	...
	class FG_Joomla_to_WordPress_Users_Authenticate {
		...
		public static function auth_signon($user, $username, $password) {
			...
			if ( self::auth_joomla($username, $password, $joomlapass) ) {
				...
				add_filter('send_password_change_email', array('FG_Joomla_to_WordPress_Users_Authenticate', 'send_password_change_email'), 10, 1); // Prevent an email to be sent
		...
		/**
		 * Blocks the email that is sent when the user logs in
		 * 
		 * @param bool $send_email Send an email or not
		 * @return bool Send an email or not
		 */
		public static function send_password_change_email($send_email) {
			return false;
		}
