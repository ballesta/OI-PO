...
if ( !class_exists('FG_Joomla_to_WordPress_Menus', false) ) {
	...
	class FG_Joomla_to_WordPress_Menus {
		...
		public function import_menus() {
			...
			$this->plugin->imported_categories = $this->plugin->get_imported_joomla_categories();
			$this->plugin->imported_sections = $this->plugin->get_imported_joomla_sections();
		...
		protected function get_menus() {
			...
			// Exclude the already imported menus
			global $wpdb;
			$ExcludeQuery = '';
			$joomla_menu_ids = $wpdb->get_col("
				SELECT meta_value FROM $wpdb->postmeta
				WHERE meta_key = '_fgj2wp_old_menu_id'
			");
			if ( is_array($joomla_menu_ids) && count($joomla_menu_ids) > 0 ) {
				$ExcludeQuery = 'AND id NOT IN (' . implode(',', $joomla_menu_ids). ')';
			}
			...
			if ( version_compare($this->plugin->plugin_options['version'], '1.5', '<=') ) {
				$name_col = 'name';
				$parent_col = 'parent';
				$order_by = 'menutype, sublevel, ordering';
			} else {
				$name_col = 'title AS name';
				$parent_col = 'parent_id AS parent';
				$order_by = 'lft';
			}
			...
			$sql = "
				SELECT id, menutype, $name_col, $alias_col, link, type, $parent_col, params
				$extra_cols
				FROM ${prefix}menu
				WHERE (type = 'url'
				OR (type = 'component'
			...
			if ( !is_null($menu_id) && !is_a($menu_id, 'WP_Error') ) {
				...
				if ( !is_null($menu_item) ) {
					$menu_data = array(
						...
						'menu-item-parent-id'	=> $this->get_parent_menu_id($menu['parent']),
					...
					if ( is_int($menu_item_id) ) {
						...
						add_post_meta($menu_item_id, '_fgj2wp_old_menu_id', $menu['id'], true);
			...
			switch ( $menu['type'] ) {
				case 'component':
					if ( preg_match('/view=article(&.*)?&id=(\d+)/', $menu['link'], $matches) ) {
						...
						switch ( $taxonomy ) {
							case 'section':
								if ( array_key_exists($jterm_id, $this->plugin->imported_sections) ) {
									$menu_item_object_id = $this->plugin->imported_sections[$jterm_id];
								} else {
									return;
								}
								break;
					...
					} elseif ( preg_match('/view=(frontpage|featured)/', $menu['link'], $matches) ) {
						// front page
		...
		/**
		 * Display the number of imported menus
		 * 
		 */
		public function display_menus_count() {
			if ( isset($this->plugin->premium_options['skip_menus']) && $this->plugin->premium_options['skip_menus'] ) {
				return;
			}
