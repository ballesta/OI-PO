...
class MindCat{
    ...
    function subcat($cat,$level,$args=array()){
		//bb
        $terms = get_terms( 'type-contenu-institutionnel', 
		                    array(
                                  'parent'=>$cat,
                                  'hirearchical'=>false,
         ...
         if(sizeof($terms)>0){
			...
			{
                $link=get_term_link( $term, 'type-contenu-institutionnel' );
				// http://oi-demo-5.ballesta.fr/type-contenu-institutionnel/les-10-types-dai/capital-systemes-dinformation/
			    // Remplace "/type-contenu-institutionnel/les-10-types-dai/"
				// Par "/contenu-instit/"
				// pour afficher le post seul qui a le même nom que la categorie
				$link_post = str_replace("/type-contenu-institutionnel/les-10-types-dai/", 
				                         "/contenu-instit/", 
							             $link);
				// echo $link_post,'<br>';						 
				$couleur_fond = $this->couleur(++$index_couleur);
                $bgcolor = isset($MindCatColors[$term->term_id]['bg'])?
				                 $MindCatColors[$term->term_id]['bg']:$couleur_fond;
                $color = isset($MindCatColors[$term->term_id]['txt'])?
				                 $MindCatColors[$term->term_id]['txt']:'#333333';
                    $ret .= '<li data-id="'.$term->term_id.'"'
 					     .  '    class="mindcat_child">'
						 .  '     <a href="'.$link_post.'" '
	...
	//bb
	function couleur($index_couleur)
	{
		// Couleurs du thème: dégradé d'orange
		$couleurs = ['#EA581D', '#F8A815', '#FCCB02', '#B5B9CD']; // , '#B5B9CD' , '#707C98'
	    $nbr_couleurs = count($couleurs);
		$c = $index_couleur % $nbr_couleurs;
		//echo $c, ' ';
		return $couleurs[$c];
	}
    ...
    function mindmap($args=''){
        ...
        if(is_string($cat) || $cat!='0'){
            ...
            if(is_numeric($cat)){
                    ...
                    //bb $term=get_term_by('slug',$cat,'category');
                    $term=get_term_by('slug',$cat,'type-contenu-institutionnel');
					//print_r($term);
            }	
            $cat=0;
            if(!is_wp_error($term)){
        ...
        //bb echo "hide_empty:$hide_empty  count:$count";
        if(false == $hide_empty || $count>0){
            $ret= '<div class="mindcat" data-size="'.$size.'">'
			    . '     <ul>'
				. '         <li class="mindcat_root">';
    
	        $ret.='<a href="' . $link
			    . '" style="background:'
				. $bgcolor.';color:'.$color.';">'.$root;
            if($cat!=0 && 1==$count){
