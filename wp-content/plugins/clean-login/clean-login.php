	...
	if ( is_singular() ) { 
		...
		if ( strpos($post->post_content, 'clean-login' ) !== false ) {
			...
			if ( isset( $_REQUEST['action'] ) && $_REQUEST['action'] == 'login' ) {
				...
				{		
					$url  = get_option( 'cl_login_redirect_url', false) ? 
								esc_url(get_option('cl_login_redirect_url'))
							: 
								esc_url( add_query_arg( 'authentication', 'success', $url ));
					//bb 
					// Redirect to calling page passed as hidden field in login form
					$redirect_to_page = $_REQUEST['redirect_to_page'];
					$url =	esc_url($redirect_to_page);	
					//echo $url, '<hr>'; // die();
					header("Location: http://www.observatoire-immateriel.com/$redirect_to_page/");
					exit;
				}
